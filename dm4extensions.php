...
Description: Automatically updates hits for WP Plugin downloads in download managers WP-DownloadManager and WordPress Download Monitor, based on hits reported by <a href="http://wordpress.org/" target="_blank">wordpress.org</a> website. Needs one of these two download managers to be installed in your WP site.
Version: 1.3
...
class dm4extensions {
  ...
  const plugin_preg_pattern = '@https?://downloads\.wordpress\.org/plugin/([a-zA-Z0-9_\-]+)(\.([0-9\.]+)|)\.zip@i';
  const plugin_like_pattern = '%://downloads.wordpress.org/plugin/%.zip';
...
//    add_action('admin_init', array($this, 'do_all'));  // for debugging-only, not waiting for the scheduled event
  ...
  function get_plugin_details($name, $types) {
    $result = wp_remote_fopen("http://wordpress.org/$types/$name/stats/");
  	$details = array();
  	if (preg_match('@(?:<th scope="row">All Time</th>.*\n.*<td>|<p class="total-downloads">Total downloads: <strong>)([0-9,]+)(?:</td>|</strong>)@i', $result, $matches)) {
